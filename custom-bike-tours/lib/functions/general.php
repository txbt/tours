...
// Change expert label to Tour Short Description
add_filter( 'gettext', 'txbt_gettext', 10, 2 );
function txbt_gettext( $translation, $original )
{
    if ( 'Excerpt' == $original && (get_post_type() == 'tour')) {
        return 'Tour Short Description';
    } else{
        $pos = strpos($original, 'Excerpts are optional hand-crafted summaries of your');
        if ($pos !== false) {
            return  'Short tour description for Tour page';
        }
    }
...
//Change featured image label to Tour Image

add_action('do_meta_boxes', 'change_image_box');
function change_image_box()
{
    remove_meta_box( 'postimagediv', 'tour', 'side' );
    add_meta_box('postimagediv', __('Tour Image'), 'post_thumbnail_meta_box', 'tour', 'normal', 'high');
}
...
function change_default_title( $title ){
     ...
     if  ( 'tour_review' == $screen->post_type ) {
          $title = 'Review Title';
     }
